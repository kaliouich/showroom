...
const i18n = {
  en: {
    ...
    heal_s4: "The API updates the gauge, Prometheus scrapes it, the alert resolves on its own. Measured end to end, no human involved: <strong>91 seconds</strong> for a fresh incident — and near-instant when a death lands while the alert group is already active, since tuning the routing below.",
    ...
    heal_insight2_title: "⏱️ Alertmanager's batching is built for humans, not scripts",
    heal_insight2: "Alertmanager groups related alerts to protect a human from notification spam: <code>groupWait</code> holds a brand-new group briefly to batch it, <code>groupInterval</code> then throttles how often it re-notifies about new alerts joining an already-active group, and <code>repeatInterval</code> resends the same notification periodically so nobody forgets it's still broken. Every Tamagotchi death shares one alert group (<code>groupBy: [alertname]</code>), so a death landing shortly after a previous one skipped the fast <code>groupWait</code> path entirely and paid the inherited <code>groupInterval</code> instead — measured at up to 186 seconds of pure batching delay before Alertmanager would even attempt delivery, on top of the alert's own <code>for: 1m</code>. That protection is pointless here: n8n reprocesses every currently-dead creature in one idempotent pass regardless of how many joined the group, so there's no notification-storm risk to guard against. Fixed by scoping <code>groupInterval: 10s</code> to the automated route alone — the parent route keeps the human-appropriate default for anything routed to a person later.",
    heal_link_routing: "📂 The routing config →",
    ...
    chaos_subtitle: 'Reading about the self-healing chain above is not the same as watching it fire. This button wipes out every living creature in the live database — not a pod, not a Kubernetes object.',
    chaos_explainer_poison: '<strong>☄️ Send a Meteor</strong> calls <code>POST /creatures/:id/kill</code> on <code>tamagotchi-api</code> for every currently living creature, setting each one\'s <code>is_alive</code> to <code>false</code> — the same state natural decay reaches on its own. That\'s what feeds <code>tamagotchi_creatures_dead_total</code>, which is exactly what <code>TamagotchiCreatureDied</code> watches. A full wipe makes the alert impossible to miss. Nothing here touches a pod: what heals it is the automation described above — Prometheus notices within a minute, Alertmanager routes on the <code>remediation: auto</code> label, and n8n revives every one of them. Usually under <strong>90 seconds</strong>, not instant on purpose — the full chain runs for real, no shortcut.',
    poison_btn_label: '☄️ Send a Meteor',
    chaos_note: "Rate-limited to one strike per minute, capped per hour — this kills every creature at once, it needs room to actually recover before it can fire again. Watch the counts above, or the Tamagotchi dashboard in the Live Infra tab, to see a meteor strike land and, a few minutes later, get reversed. Want to remove a creature permanently instead? That's a per-creature action in the app itself — the opposite of Adopt, not a chaos event.",
    chaos_demo_link: 'the app itself',
    poison_status_working: 'Calling down a meteor…', poison_status_done: '{count} creature(s) poisoned ☄️ — watch for the revival, usually under 90s',
  ...
  fr: {
    ...
    heal_s4: "L'API met à jour la jauge, Prometheus la collecte, l'alerte se résout seule. Mesuré de bout en bout, sans intervention humaine : <strong>91 secondes</strong> pour un incident frais — et quasi instantané quand une mort survient pendant que le groupe d'alerte est déjà actif, depuis le réglage du routage ci-dessous.",
    ...
    heal_insight2_title: "⏱️ Le regroupement d'Alertmanager est pensé pour des humains, pas des scripts",
    heal_insight2: "Alertmanager regroupe les alertes liées pour protéger un humain du spam de notifications : <code>groupWait</code> retient brièvement un groupe tout neuf pour le regrouper, <code>groupInterval</code> limite ensuite la fréquence des notifications concernant de nouvelles alertes qui rejoignent un groupe déjà actif, et <code>repeatInterval</code> renvoie la même notification périodiquement pour que personne n'oublie que c'est toujours cassé. Chaque mort de créature Tamagotchi partage un seul groupe d'alerte (<code>groupBy: [alertname]</code>), donc une mort survenant peu après une précédente sautait entièrement le chemin rapide de <code>groupWait</code> et payait le <code>groupInterval</code> hérité à la place — mesuré jusqu'à 186 secondes de pur délai de regroupement avant qu'Alertmanager tente ne serait-ce qu'une livraison, en plus du <code>for: 1m</code> propre à l'alerte. Cette protection n'a aucun sens ici : n8n retraite toutes les créatures actuellement mortes en un seul passage idempotent, peu importe combien en rejoignent le groupe, donc aucun risque de tempête de notifications à couvrir. Corrigé en scopant <code>groupInterval: 10s</code> à la seule route automatisée — la route parente garde la valeur par défaut adaptée à un humain pour toute future route qui en atteindrait un.",
    heal_link_routing: "📂 La config de routage →",
    ...
    chaos_subtitle: "Lire sur la chaîne d'auto-réparation ci-dessus, ce n'est pas la même chose que la voir se déclencher. Ce bouton élimine toutes les créatures vivantes de la base en production — pas un pod, pas un objet Kubernetes.",
    chaos_explainer_poison: "<strong>☄️ Envoyer une Météorite</strong> appelle <code>POST /creatures/:id/kill</code> sur <code>tamagotchi-api</code> pour chaque créature actuellement vivante, mettant <code>is_alive</code> à <code>false</code> pour chacune — le même état que la décroissance naturelle atteint toute seule. C'est ce qui alimente <code>tamagotchi_creatures_dead_total</code>, exactement ce que surveille <code>TamagotchiCreatureDied</code>. Une élimination totale rend l'alerte impossible à manquer. Rien ici ne touche un pod : ce qui guérit les créatures, c'est l'automatisation décrite plus haut — Prometheus le remarque en moins d'une minute, Alertmanager route sur le label <code>remediation: auto</code>, et n8n les ranime toutes. Généralement en moins de <strong>90 secondes</strong>, pas instantané, exprès — la chaîne complète tourne pour de vrai, sans raccourci.",
    poison_btn_label: '☄️ Envoyer une Météorite',
    chaos_note: "Limité à une frappe par minute, plafonné par heure — ça tue toutes les créatures d'un coup, il faut le temps que la population se refasse avant de pouvoir refrapper. Observe les compteurs ci-dessus, ou le dashboard Tamagotchi dans l'onglet Live Infra, pour voir une météorite frapper puis, quelques minutes plus tard, être annulée. Envie de supprimer une créature définitivement plutôt ? C'est une action par créature dans l'application elle-même — l'opposé d'Adopter, pas un événement chaos.",
    chaos_demo_link: "l'application elle-même",
    poison_status_working: "Appel d'une météorite…", poison_status_done: '{count} créature(s) empoisonnée(s) ☄️ — surveille la résurrection, généralement en moins de 90s',
...
// ---- Chaos Button ----
// Matches the 60s cooldown in website/server.js's poisonLimiter — the
...
// be told no.
const CHAOS_CLIENT_COOLDOWN_MS = 60000;

const poisonBtn = document.getElementById('poisonBtn');
if (poisonBtn) {
  poisonBtn.addEventListener('click', async () => {
    const statusEl = document.getElementById('poisonStatus');
    poisonBtn.disabled = true;
    statusEl.className = 'chaos-panel__status';
    statusEl.textContent = t('poison_status_working');
    try {
      const res = await fetch('/api/chaos/poison-creature', { method: 'POST' });
      ...
      if (res.status === 429) {
        ...
        statusEl.textContent = t('poison_status_done').replace('{count}', data.count ?? 0);
      ...
      setTimeout(() => { poisonBtn.disabled = false; }, CHAOS_CLIENT_COOLDOWN_MS);
    }
  });
}
